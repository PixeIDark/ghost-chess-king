...
export class LobbyService {
  ...
  constructor(
    ...
    private readonly messageService: MessageService<ChatMessage>
  ) {}
  ...
  saveLobbyMessage(odId: string, message: string): void {
    const user = this.users.get(odId);
    if (!user) return;
    ...
    this.messageService.saveMessage(chatData);
  }

  async loadLobbyMessageHistory(): Promise<ChatMessage[]> {
    return await this.messageService.loadMessageHistory();
  }

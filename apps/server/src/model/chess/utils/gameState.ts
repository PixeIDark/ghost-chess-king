...
export const isSquareAttacked = (board: Board, targetRow: number, targetCol: number, defenderColor: Side): boolean => {
  ...
  for (let row = 0; row < BOARD_SIZE; row++) {
    for (let col = 0; col < BOARD_SIZE; col++) {
      ...
      if (canPieceAttack(board, row, col, targetRow, targetCol, cell)) return true;
    }
  }
  return false;
};

export const isCheckmate = (board: Board, fen: string, color: Side): boolean => {
  if (!isCheck(board, color)) return false;
  return hasNoLegalMoves(board, fen, color);
};

export const isStalemate = (board: Board, fen: string, color: Side): boolean => {
  if (isCheck(board, color)) return false;
  return hasNoLegalMoves(board, fen, color);
};
...
export const wouldBeCheck = (board: Board, from: Square, to: Square, color: Side, fen: string): boolean => {
  const newBoard = simulateMove(board, from, to, fen);
  return isCheck(newBoard, color);
};

export const hasNoLegalMoves = (board: Board, fen: string, color: Side): boolean => {
  for (let row = 0; row < BOARD_SIZE; row++) {
    for (let col = 0; col < BOARD_SIZE; col++) {
      const cell = board[row][col];

...
import { getCastlingRights, getEnPassantTarget } from "@/model/chess/utils/fenUtils";

export const getValidMoves = (board: Board, fen: string, from: Square): Square[] => {
  ...
  if (piece.type === "pawn") moves.push(...getEnPassantMoves(fen, row, col, piece.color));

  if (piece.type === "king") moves.push(...getCastlingMoves(board, fen, piece.color));

  return moves.filter((move) => !wouldBeCheck(board, from, move, piece.color, fen));
};

const getBasicMoves = (board: Board, row: number, col: number, piece: Piece): Square[] => {
  const moves: Square[] = [];
  ...
  for (let toRow = 0; toRow < BOARD_SIZE; toRow++) {
    for (let toCol = 0; toCol < BOARD_SIZE; toCol++) {
      ...
      if (canPieceAttack(board, row, col, toRow, toCol, piece)) moves.push(indicesToSquare(toRow, toCol));
    }
  }
...
const getEnPassantMoves = (fen: string, row: number, col: number, color: Side): Square[] => {
  const moves: Square[] = [];
  ...
  if (Math.abs(col - epCol) === 1 && row + direction === epRow) moves.push(enPassant as Square);
...
const getCastlingMoves = (board: Board, fen: string, color: Side): Square[] => {
  ...
  const canKingSide =
    ((color === "white" && castling.includes("K")) || (color === "black" && castling.includes("k"))) &&
  ...
  const canQueenSide =
    ((color === "white" && castling.includes("Q")) || (color === "black" && castling.includes("q"))) &&

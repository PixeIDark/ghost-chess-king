...
export class ChessTimer implements IChessTimer {
  ...
  public on<K extends TimerEventKey>(event: K, listener: (data: TimerEventMap[K]) => void): void {
    if (!this.listeners.has(event)) this.listeners.set(event, new Set());
    this.listeners.get(event)!.add(listener);
  }

  public off<K extends TimerEventKey>(event: K, listener: (data: TimerEventMap[K]) => void): void {
    this.listeners.get(event)?.delete(listener);
  }

  public start(startTurn: Side): void {
    this.currentTurn = startTurn;
    this.lastUpdateTime = Date.now();
    this.timerInterval = setInterval(() => this.tick(), 100);
  }

  public switchTurn(nextTurn: Side): void {
  ...
  public getTime(): { whiteTime: number; blackTime: number } {
    return {
      whiteTime: Math.max(0, this.whiteTime),
      blackTime: Math.max(0, this.blackTime),
    };
